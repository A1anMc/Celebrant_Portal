...
import { ArrowLeft, Edit, Trash2, User, Heart, Phone, Mail, MapPin, Calendar, FileText } from 'lucide-react';
...
export default function CoupleDetailPage() {
  ...
  const handleDelete = async () => {
    ...
    const confirmed = confirm(`Are you sure you want to delete ${couple.full_names || `${couple.partner_1_first_name} ${couple.partner_1_last_name} & ${couple.partner_2_first_name} ${couple.partner_2_last_name}`}? This action cannot be undone.`);
      ...
      <div className="p-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <Card className="p-6">
            ...
            <div className="space-y-3">
              <div>
                <span className="text-sm font-medium text-gray-700">Relationship Started:</span>
                <span className="text-sm text-gray-600 ml-2">{formatDate(couple.relationship_start_date)}</span>
              </div>
              {couple.previous_marriages && (
                <div>
                  <span className="text-sm font-medium text-gray-700">Previous Marriages:</span>
                  <span className="text-sm text-gray-600 ml-2 capitalize">{couple.previous_marriages.replace('_', ' ')}</span>
                </div>
              )}
              <div>
                <span className="text-sm font-medium text-gray-700">Primary Contact:</span>
                <span className="text-sm text-gray-600 ml-2 capitalize">{couple.primary_contact.replace('_', ' ')}</span>
              </div>
              <div>
                <span className="text-sm font-medium text-gray-700">Preferred Contact Method:</span>
                <span className="text-sm text-gray-600 ml-2 capitalize">{couple.preferred_contact_method}</span>
              </div>
            </div>
          </Card>
            ...
            <div className="space-y-4">
              {couple.referral_source && (
                <div>
                  ...
                  <p className="text-sm text-gray-600 mt-1">{couple.referral_source}</p>
                </div>
              )}
              {couple.notes && (
                <div>
                  <span className="text-sm font-medium text-gray-700">Notes:</span>
                  <p className="text-sm text-gray-600 mt-1">{couple.notes}</p>
                </div>
              )}
              {couple.internal_notes && (
                <div>
                  <span className="text-sm font-medium text-gray-700">Internal Notes:</span>
                  <p className="text-sm text-gray-600 mt-1">{couple.internal_notes}</p>
                </div>
              )}
              <div>
                <span className="text-sm font-medium text-gray-700">Marketing Consent:</span>
                <span className="text-sm text-gray-600 ml-2 capitalize">{couple.marketing_consent.replace('_', ' ')}</span>
              </div>
              {couple.updated_at && (
                <div>
                  <span className="text-sm font-medium text-gray-700">Last Updated:</span>
                  <span className="text-sm text-gray-600 ml-2">{formatDate(couple.updated_at)}</span>
                </div>
              )}
            </div>
          </Card>
